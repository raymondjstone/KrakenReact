    ...
    <div style={{ position: 'fixed', inset: 0, background: 'var(--overlay-bg)', zIndex: 9999 }}>
      ...
      >
        ...
        >
          ...
          <button onClick={() => onClose(false)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', fontSize: 20, cursor: 'pointer', lineHeight: 1 }} title="Close">x</button>
        </div>
                ...
                {bracketEnabled && (
                  <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      {price && Number(bracketStopPct) > 0 && <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>@ ${(Number(price) * (1 - Number(bracketStopPct) / 100)).toFixed(2)}</span>}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      {price && Number(bracketTpPct) > 0 && <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>@ ${(Number(price) * (1 + Number(bracketTpPct) / 100)).toFixed(2)}</span>}
                    </div>
